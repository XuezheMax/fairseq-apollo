...
#pragma unroll
  for (int64_t i = 0; i < kElementsPerThread / 4; ++i) {
    const int64_t idx = i * blockDim.x + threadIdx.x;
    const cuda_utils::BF16x4 v4 = src4[idx];
    dst[i * 4 + 0] =
...
#pragma unroll
  for (int64_t i = 0; i < kElementsPerThread / 8; ++i) {
    const int64_t idx = i * blockDim.x + threadIdx.x;
    const cuda_utils::BF16x8 v8 = src8[idx];
    dst[i * 8 + 0] =
...
__inline__ __device__ void Load(const SRC* __restrict__ src, int64_t size,
  ...
  if constexpr (std::is_same<SRC, float>::value &&
    ...
    if (kElementsPerThread % 4 == 0 &&
        reinterpret_cast<std::uintptr_t>(src) % sizeof(float4) == 0 &&
        stride % 4 == 0) {
      LoadFP32x4Impl<kElementsPerThread>(src, size, default_value, dst);
    } else if (kElementsPerThread % 2 == 0 &&
               reinterpret_cast<std::uintptr_t>(src) % sizeof(float2) == 0 &&
               stride % 2 == 0) {
    ...
    if (kElementsPerThread % 2 == 0 &&
        reinterpret_cast<std::uintptr_t>(src) % sizeof(__half2) == 0 &&
        stride % 2 == 0) {
    ...
    if (kElementsPerThread % 8 == 0 &&
        reinterpret_cast<std::uintptr_t>(src) % sizeof(cuda_utils::BF16x8) ==
            0 &&
        stride % 8 == 0) {
      LoadBF16x8Impl<kElementsPerThread>(src, size, default_value, dst);
               ...
               reinterpret_cast<std::uintptr_t>(src) %
                       sizeof(cuda_utils::BF16x4) ==
                   0 &&
               stride % 4 == 0) {
      LoadBF16x4Impl<kElementsPerThread>(src, size, default_value, dst);
    } else if (kElementsPerThread % 2 == 0 &&
               reinterpret_cast<std::uintptr_t>(src) % sizeof(__nv_bfloat162) ==
                   0 &&
               stride % 2 == 0) {
...
__inline__ __device__ void Save(const SRC* __restrict__ src, int64_t size,
  ...
  if constexpr (std::is_same<SRC, float>::value &&
                std::is_same<DST, float>::value) {
    if (kElementsPerThread % 4 == 0 &&
        reinterpret_cast<std::uintptr_t>(dst) % sizeof(float4) == 0 &&
        size % 4 == 0) {
      SaveFP32x4Impl<kElementsPerThread>(src, size, dst);
    } else if (kElementsPerThread % 2 == 0 &&
               reinterpret_cast<std::uintptr_t>(dst) % sizeof(float2) == 0 &&
               size % 2 == 0) {
    ...
    if (kElementsPerThread % 2 == 0 &&
        reinterpret_cast<std::uintptr_t>(dst) % sizeof(__half2) == 0 &&
        size % 2 == 0) {
    ...
    if (kElementsPerThread % 8 == 0 &&
        reinterpret_cast<std::uintptr_t>(dst) % sizeof(cuda_utils::BF16x8) ==
            0 &&
        size % 8 == 0) {
      ...
      SaveBF16x4Impl<kElementsPerThread>(src, size, dst);
               ...
               reinterpret_cast<std::uintptr_t>(dst) % sizeof(__nv_bfloat162) ==
                   0 &&
               size % 2 == 0) {
