...
class LunaHubInterface(nn.Module):
    """A simple PyTorch Hub interface to Luna.

    Usage: https://github.com/pytorch/fairseq/tree/master/examples/luna
    """
    ...
    def encode(self, sentence: str, *addl_sentences, no_separator=False) -> torch.LongTensor:
        ...
        requires leading spaces. For example::
            ...
            >>> luna.encode('Hello world').tolist()
            [0, 31414, 232, 2]
            >>> luna.encode(' world').tolist()
            [0, 232, 2]
            >>> luna.encode('world').tolist()
            [0, 8331, 2]
        """
    ...
    def extract_all_features(self, tokens: torch.LongTensor, return_all_hiddens: bool = False) -> Tuple[torch.Tensor, torch.Tensor]:
        if tokens.dim() == 1:
        ...
        if return_all_hiddens:
            ...
            for inner_state in inner_states:
                features.append(inner_state[0].transpose(0, 1))
                packed_features.append(inner_state[1].transpose(0, 1))
        else:
            packed_features = extra['packed_features']

        return features, packed_features
    ...
    def extract_features(self, tokens: torch.LongTensor, return_all_hiddens: bool = False) -> torch.Tensor:
        ...
        if return_all_hiddens:
            # convert from T x B x C -> B x T x C
            inner_states = extra['inner_states']
            return [inner_state[0].transpose(0, 1) for inner_state in inner_states]
        else:
    ...
    def extract_features_aligned_to_words(self, sentence: str, return_all_hiddens: bool = False) -> torch.Tensor:
        """Extract Luna features, aligned to spaCy's word-level tokenizer."""
