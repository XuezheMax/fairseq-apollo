...
class LRAModel(FairseqEncoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--moving-act', choices=['rmsnorm', 'silu'])
...
def mega_lra_listop(args):
    ...
    args.encoder_hidden_dim = getattr(args, 'encoder_hidden_dim', 168)
    args.z_dim = getattr(args, 'z_dim', 64)
    args.n_dim = getattr(args, 'n_dim', 16)
    args.encoder_layers = getattr(args, 'encoder_layers', 6)
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 84)
...
def mega_lra_imdb(args):
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.layer_type = getattr(args, 'layer_type', 'mega')
    args.encoder_hidden_dim = getattr(args, 'encoder_hidden_dim', 280)
    args.z_dim = getattr(args, 'z_dim', 72)
    args.n_dim = getattr(args, 'n_dim', 16)
    args.encoder_layers = getattr(args, 'encoder_layers', 4)
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 128)
    args.chunk_size = getattr(args, 'chunk_size', -1)
