...
class MegaSentenceEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.mega_layer = self.build_mega_layer(embedding_dim, hidden_dim, z_dim, n_dim,
                                                ...
                                                rel_pos_bias, max_positions, init_mode, export)

        if ffn_hidden_dim is not None and ffn_hidden_dim > 0:
            self.nffn = self.build_nffn_layer(embedding_dim, ffn_hidden_dim, dropout, hidden_dropout, activation, init_mode)
        else:
            self.nffn = None
    ...
    def build_nffn_layer(self, embedding_dim, ffn_hidden_dim, dropout, hidden_dropout, activation, init_mode):
