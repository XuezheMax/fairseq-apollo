...
class EMAGatedAttention(nn.Module):
    ...
    def __init__(
        ...
        self.ema_decay = nn.Parameter(torch.Tensor(embed_dim))
    ...
    def compute_kernel(self, length: int):
        # D x 1
        gamma = torch.sigmoid(self.ema_decay).unsqueeze(1)
        # D x N
    ...
    def ema_forward(
        ...
        if self.bidirectional:
            ...
            out = (out + out2.flip(-1)) * 0.5

        # B x D x N -> N x B x D
        out = (out.permute(2, 0, 1) + residual) * 0.5
        return out
    ...
    def forward(
        ...
        # N x B x D
        x = self.ema_forward(x, incremental_state)
