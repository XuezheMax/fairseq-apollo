...
class MultiHeadComplexEMA(nn.Module):
    ...
    def step(self, x, length, hx=None):
        ...
        if hx is not None:
            ...
            ox = torch.einsum('bdn,dnl->bdl', hx, k).real
    ...
    def one_step(self, x, hx=None):
        ...
        if hx is not None:
            h = h + q.squeeze(-1) * hx
        # B x D
        out = torch.einsum('bdn,dn->bd', h, gamma).real
        # 1 x B x D, B x D x N
