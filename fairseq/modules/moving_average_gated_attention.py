...
class MovingAverageGatedAttention(nn.Module):
    ...
    def __init__(
        ...
        self.mx_proj = nn.Linear(embed_dim, zdim + 3 * embed_dim)
    ...
    def forward(
        ...
        u, z, r, hx = torch.split(base, [self.embed_dim, self.zdim, self.embed_dim, self.embed_dim], dim=-1)
        # L x B x D
        u = torch.sigmoid(u)
        # L x B x S
        ...
        # L x B x E
        attn = self.hidden_dropout(attn)
        # L x B x E -> L x B x D
        h = self.h_proj(attn) * r
        h = self.activation(hx + h)
        h = self.dropout(h)
        # L x B x D
