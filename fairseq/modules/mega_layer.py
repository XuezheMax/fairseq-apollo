...
class MegaDecoderLayer(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.dropout_module = FairseqDropout(args.dropout, module_name=self.__class__.__name__)
        self.mega_layer1 = self.build_mega_layer(self.embed_dim, args)
        self.cross_attn = self.build_cross_attn(self.embed_dim, args)
        self.mega_layer2 = self.build_mega_layer(self.embed_dim, args)
    ...
    def forward(
        ...
        Returns:
            encoded output of shape `(seq_len, batch, embed_dim)`
        """
        x, _ = self.mega_layer1(x=x, padding_mask = decoder_padding_mask,
        ...
        x, _ = self.mega_layer2(x=x, padding_mask = decoder_padding_mask,
                                incremental_state=incremental_state,
                                need_weights=False, attn_mask=attn_mask)
        x = self.dropout_module(x)
